...
def main() -> int:
    ...
    if CYCLE_DIR.exists():
        ...
        # Collect the existing per-target losses without assuming a single
        # report schema. This is measurement only; no reconstruction occurs.
        step7_rows = []

        def collect_loss_rows(value):
            if isinstance(value, dict):
                sid = value.get("sample_id", value.get("target_id"))
                loss = value.get("total_loss", value.get("loss"))
                ...
                if isinstance(loss, (int, float)):
                    step7_rows.append({
                        ...
                        "loss": float(loss),
                        "weight": 1.0 / 25.0,
                        "weighted_loss": float(loss) / 25.0,
                    })

                for child in value.values():
                    if isinstance(child, (dict, list)):
                        collect_loss_rows(child)

            elif isinstance(value, list):
                for child in value:
                    collect_loss_rows(child)

        collect_loss_rows(existing_report)

        # Deduplicate rows when a report nests the same target/result more
        # than once.
        unique_rows = []
        seen = set()
        for row in step7_rows:
            key = (row["sample_id"], row["loss"])
            if key not in seen:
                seen.add(key)
        ...
        if not unique_rows:
            print(
                "[STEP 7 ERROR] No per-target numeric losses "
                "were found in CYCLE_v0.1."
            )
            return 1

        # Prefer the frozen 001..025 reference set.
        filtered = []
        for row in unique_rows:
            ...
            if isinstance(sid, int) and 1 <= sid <= 25:
                row = dict(row)
                row["sample_id"] = sid
                filtered.append(row)
        ...
        if len(filtered) != 25:
            print(
                "[STEP 7 ERROR] Expected exactly 25 reference losses; "
                f"found {len(filtered)}."
            )
            return 1

        filtered.sort(key=lambda r: r["sample_id"])

        losses = [row["loss"] for row in filtered]
        mean_loss = sum(losses) / 25.0
        baseline = 0.32439998436125367
        improvement = baseline - mean_loss
        ...
        step7_report = {
            ...
            "measurement": {
                ...
                "difference_from_baseline": mean_loss - baseline,
                "improvement_over_baseline": improvement,
                "better_than_baseline": mean_loss < baseline,
            },
            "safety": {
                "video_used": False,
        ...
        print("Measured mean loss:", mean_loss)
