...
def build_assets(sd):
 d=sd/FINAL; d.mkdir(parents=True,exist_ok=True); s=source(sd); x0,y0,x1,y1=bbox(alpha(s)); m=Image.fromarray(alpha(s)[y0:y1,x0:x1],'L')
 ...
 im=Image.new('RGBA',m.size,(255,255,255,255)); im.paste((0,0,0,255),mask=m); im.convert('RGB').save(d/ASSETS[0],optimize=True)
 im=Image.new('RGBA',m.size,(0,0,0,255)); im.paste((255,255,255,255),mask=m); im.convert('RGB').save(d/ASSETS[2],optimize=True)
 return {'source':s.name,'crop':[x0,y0,x1,y1],'output_size':[m.width,m.height],'upscaled':False}
def pv(v):
