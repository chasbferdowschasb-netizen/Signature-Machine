...
"""
Signature Machine
Stage 2 - Deep Visual Analyzer v0.6.2
...
class DeepVisualAnalyzer:
    ...
    VERSION = "0.6.2"
    PIPELINE_VERSION = "signature_only_v6_2_robust_polarity_nonempty"
    ...
    def __init__(
        ...
        debug: bool = False,
        debug_dir: str | Path = "test_debug_v06_2",
    ):
    ...
    def _detect_polarity(self, gray: np.ndarray, alpha: np.ndarray) -> tuple[np.ndarray, dict]:
        """Robust polarity detection for black/white/transparent exports.

        v0.6.1 relied on local residual contrast. On several test samples the
        residual was dominated by the page/layout itself, so the true signature
        candidate collapsed to zero. v0.6.2 uses the image border as the primary
        background estimate and keeps alpha only when alpha contains meaningful
        visible pixels.
        """
        h, w = gray.shape
        ...
        if not np.any(visible):
            raise ValueError("No visible RGB/alpha pixels available for polarity detection")
        ...
        border_mask[:, :band] = True
        border_mask[:, -band:] = True
        border_visible = border_mask & visible
        border_values = gray[border_visible]
        border_median = float(np.median(border_values)) if border_values.size else float(np.median(gray[visible]))

        center_mask = np.zeros_like(gray, dtype=bool)
        y0, y1 = int(0.06 * h), int(0.94 * h)
        x0, x1 = int(0.06 * w), int(0.94 * w)
        center_mask[y0:y1, x0:x1] = True
        center_visible = center_mask & visible
        center_values = gray[center_visible]
        center_median = float(np.median(center_values)) if center_values.size else border_median

        q05, q20, q50, q80, q95 = [
            float(v) for v in np.quantile(gray[visible], [0.05, 0.20, 0.50, 0.80, 0.95])
        ]

        # Strong polarity from the physical border. This handles the common
        # white-ink-on-black and black-ink-on-white test exports directly.
        if border_median <= 100.0:
            threshold = max(25.0, min(200.0, border_median + 25.0))
            # If the image contains a substantial gray foreground, use q80 but
            # never let the threshold become so high that anti-aliased strokes die.
            if q80 > threshold + 35.0:
                threshold = min(threshold + 15.0, q80 * 0.55)
            mask = (gray >= threshold) & visible
            polarity = "light_ink_on_dark_background"
        elif border_median >= 175.0:
            threshold = min(230.0, max(30.0, border_median - 25.0))
            if q20 < threshold - 35.0:
                threshold = max(threshold - 15.0, q20 * 1.45)
            mask = (gray <= threshold) & visible
            polarity = "dark_ink_on_light_background"
        else:
            # Ambiguous/photographic border: compare both global polarities.
            otsu = self._otsu(gray[visible])
            dark = (gray <= otsu) & visible
            light = (gray > otsu) & visible
            dark_border = float(np.mean(dark[border_mask]))
            light_border = float(np.mean(light[border_mask]))
            dark_center = float(np.mean(dark[center_mask]))
            light_center = float(np.mean(light[center_mask]))
            dark_score = dark_center * 3.0 - dark_border * 8.0
            light_score = light_center * 3.0 - light_border * 8.0
            if dark_score >= light_score:
                mask = dark
                polarity = "dark_otsu_ambiguous_background"
                threshold = otsu
            else:
                mask = light
                polarity = "light_otsu_ambiguous_background"
                threshold = otsu

        # Add chromatic strokes that grayscale thresholding can miss.
        rgb = None
        try:
            # gray is enough for the normal path; color contrast is intentionally
            # omitted here because _detect_polarity receives gray only.
            pass
        except Exception:
            pass

        # Guard against a polarity that selects almost the entire canvas.
        ratio = float(np.mean(mask[visible]))
        if ratio > 0.60:
            otsu = self._otsu(gray[visible])
            dark = (gray <= otsu) & visible
            light = (gray > otsu) & visible
            db = float(np.mean(dark[border_mask]))
            lb = float(np.mean(light[border_mask]))
            dc = float(np.mean(dark[center_mask]))
            lc = float(np.mean(light[center_mask]))
            if dc * 3.0 - db * 8.0 >= lc * 3.0 - lb * 8.0:
                mask = dark
                polarity = "dark_otsu_guard"
            else:
                mask = light
                polarity = "light_otsu_guard"
            threshold = otsu
            ratio = float(np.mean(mask[visible]))
        ...
        return mask.astype(np.uint8), {
            ...
            "center_median": round(center_median, 4),
            "q05": round(q05, 4),
            "q20": round(q20, 4),
            "q50": round(q50, 4),
            "q80": round(q80, 4),
            "q95": round(q95, 4),
            "border_fraction": round(float(np.mean(mask[border_mask])), 6),
            "foreground_ratio_before_cleanup": round(ratio, 6),
    ...
    def extract_signature_mask(self, image_path: Path) -> tuple[np.ndarray, dict]:
        ...
        candidate_pixels_raw = int(np.count_nonzero(candidate))

        candidate = self._morph(candidate)
        candidate_pixels_morphed = int(np.count_nonzero(candidate))

        cleaned, cleanup = self._cleanup_layout(candidate)
        cleaned_pixels = int(np.count_nonzero(cleaned))
        fallback_stage = None

        # If cleanup erased everything, do NOT report a successful black mask.
        # First retry with the pre-cleanup candidate.
        if cleaned_pixels == 0 and candidate_pixels_morphed > 0:
            cleaned = candidate.copy()
            cleaned_pixels = int(np.count_nonzero(cleaned))
            fallback_stage = "pre_cleanup_candidate"

        # Small component filter, but only when it leaves real signal behind.
        if ndi is not None and cleaned_pixels > 0:
            ...
            if np.any(filtered):
                ...
                fallback_stage = fallback_stage or "post_filter_candidate"
        ...
        if signature_pixels == 0:
            raise ValueError(
                "EMPTY_SIGNATURE_MASK: extraction produced zero pixels. "
                f"raw={candidate_pixels_raw}, morphed={candidate_pixels_morphed}, "
                f"cleanup={cleaned_pixels}, polarity={polarity.get('polarity')}"
            )
        ...
        diagnostics = {
            ...
            **cleanup,
            ...
            "candidate_pixels_raw": candidate_pixels_raw,
            "candidate_pixels_after_morph": candidate_pixels_morphed,
    ...
    def analyze_image(self, image_path: str | Path) -> dict:
        ...
        if not np.any(signature_mask):
            raise ValueError("EMPTY_SIGNATURE_MASK: analyzer returned an all-zero mask")
