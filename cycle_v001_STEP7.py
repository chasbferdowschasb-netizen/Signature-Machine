...
def main() -> int:
    ...
    # ---------------------------------------------------------------
    # Never overwrite an existing cycle
    # ---------------------------------------------------------------
...
if __name__ == "__main__":
    ...
    # ================================================================
    # STEP 7 — LOSS MEASUREMENT
    # ================================================================
    #
    # Integrated into Cycle v0.1.
    # Existing reconstruction/evaluation responsibilities remain intact.
    #
    # Rules:
    #   - Reference set is frozen 001..025.
    #   - Every reference has equal weight 1/25 = 0.04.
    #   - No video.
    #   - No random points.
    #   - No generated samples.
    #   - Sample 026 is not created.
    #   - MODEL v001 is not created.
    #
    # STEP 7 measures and reports the loss already produced by the
    # existing leave-one-out reconstruction cycle. It does not create
    # a second reconstruction engine.

    STEP7_VERSION = "0.1"
    STEP7_BASELINE_LOSS = 0.32439998436125367
    STEP7_REFERENCE_COUNT = 25
    STEP7_REFERENCE_WEIGHT = 1.0 / STEP7_REFERENCE_COUNT

    def measure_step7_loss(
        self,
        reconstruction_results,
    ):
        """
        Measure STEP 7 loss from the existing reconstruction results.

        This method deliberately consumes the current Cycle output instead
        of introducing another reconstruction/generation path.
        """
        if not isinstance(reconstruction_results, list):
            raise TypeError(
                "reconstruction_results must be a list"
            )
            ...
            per_sample.append(
                {
                    "sample_id": sample_id,
                    "loss": loss,
                    "weight": self.STEP7_REFERENCE_WEIGHT,
                    "weighted_loss":
                        loss * self.STEP7_REFERENCE_WEIGHT,
                }
            )

        if not losses:
            raise ValueError(
                "STEP 7 could not find numeric losses in the "
                "existing reconstruction results."
            )

        weighted_mean = sum(losses) / len(losses)

        return {
            "version": self.STEP7_VERSION,
            "reference_policy": {
                "first_sample": 1,
                "last_sample": 25,
                "reference_count": self.STEP7_REFERENCE_COUNT,
                "equal_weight": True,
                "weight_per_reference":
                    self.STEP7_REFERENCE_WEIGHT,
                "priority": "none",
            },
            "baseline": {
                "cycle": "CYCLE_v0.1",
                "mean_loss": self.STEP7_BASELINE_LOSS,
            },
            "measurement": {
                "evaluated_samples": len(losses),
                "mean_loss": weighted_mean,
                "weighted_mean_loss": weighted_mean,
                "difference_from_baseline":
                    weighted_mean -
                    self.STEP7_BASELINE_LOSS,
