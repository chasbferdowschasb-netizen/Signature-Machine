...
class DeepVisualAnalyzer:
    ...
    VERSION = "0.5"
    PIPELINE_VERSION = "signature_only_v4_polarity_graph"
    ...
    def __init__(
        ...
        debug: bool = False,
        debug_dir: str | Path = "test_debug_v05",
    ):
    ...
    def _cleanup_layout(self, mask: np.ndarray) -> tuple[np.ndarray, dict]:
        ...
        for c in components:
            ...
            if c["touches_border"] and c["bbox_ratio"] > 0.015:
                ...
                if horizontal_layout or vertical_layout or giant_layout:
                    removed += 1
                    continue
    ...
    def _save_debug(self, image_path: Path, foreground: np.ndarray, signature: np.ndarray, skeleton: np.ndarray) -> dict:
        ...
        Image.fromarray((foreground * 255).astype(np.uint8)).save(fg_path)
        Image.fromarray((signature * 255).astype(np.uint8)).save(mask_path)
        Image.fromarray((skeleton * 255).astype(np.uint8)).save(sk_path)

        base = np.zeros((*signature.shape, 3), dtype=np.uint8)
        base[:, :, :] = 0
        base[signature > 0] = (220, 220, 220)
        base[skeleton > 0] = (255, 255, 255)
