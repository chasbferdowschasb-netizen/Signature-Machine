...
def main():
    ...
    if weight is not None:
        ...
        policy_values = []
        for obj in (freeze_data if "freeze_data" in locals() else None,
                    manifest_data if "manifest_data" in locals() else None):
            if isinstance(obj, dict):
                for key in (
                    ...
                    value = obj.get(key)
                    if finite(value):
                        policy_values.append(float(value))
        if policy_values:
            ok = any(abs(v - EXPECTED_WEIGHT) <= 1e-9 for v in policy_values)
            checks.append(("Equal weight = 0.04", ok))
        else:
            warnings.append(
                "Equal-weight field not duplicated in STEP8 or STEP1/STEP2 artifacts."
            )
                ...
                if isinstance(value, list):
                    for item in value:
                        if isinstance(item, int):
                            ...
                            import re
                            m = re.search(r"(?:sample[_ -]?)?(\d{1,6})$", item, re.I)
                            if m:
                                found.append(int(m.group(1)))
                        elif isinstance(item, dict):
                            found.extend(extract_reference_ids(item))
                elif isinstance(value, (dict, list)):
                    found.extend(extract_reference_ids(value))
