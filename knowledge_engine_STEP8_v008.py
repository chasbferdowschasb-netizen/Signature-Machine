...
class SignatureKnowledgeEngine:
    ...
    def _run_step8_iterative_learning(
        ...
        *,
        ...
        """
        STEP 8 v0.4 — Leave-One-Out trajectory-style rule learning.

        Important architectural rule:
        this continues the existing Knowledge Engine responsibility. It does
        not create another engine and it does not modify the frozen samples.

        The learner does NOT optimize toward the target sample directly.
        For each target, its reconstruction is produced only from the other
        24 frozen references. The target is used only as an evaluation loss.

        The learned parameters are per-root style blending coefficients:
            reconstruction =
                global_style + alpha * (leave_one_out_peer_consensus
                                        - global_style)

        alpha starts at 0 and is learned iteratively. This makes STEP 8 an
        actual leave-one-out learning problem rather than simply declaring
        the global mean to be the learned model.
        """
        ...
        flat_rows = [self._step8_flat_features(row) for row in rows]

        # Feature roots keep indexed sequence fields from dominating the
        # learning objective. All reference samples still have exactly
        # equal weight 1/25.
        ...
        feature_keys = sorted({
            ...
            for key in row
            if key in stats and stats[key]["learnable"]
        })
        ...
        roots = sorted({root_of(key) for key in feature_keys})
        ...
        # Build standardized target values and standardized peer consensus.
        # The peer consensus for target i never contains target i.
        standardized_targets = []
        peer_consensus = []

        for target_index, target in enumerate(flat_rows):
            target_z = {}
            consensus_z = {}
            ...
            for key in feature_keys:
                info = stats[key]
                scale = info["scale"]
                target_value = target.get(key, info["mean"])

                target_z[key] = (
                    (target_value - info["mean"]) / scale
                    if info["learnable"]
                    else 0.0
                )
                ...
                peers = [
                    flat_rows[j][key]
                    for j in range(len(flat_rows))
                    if j != target_index and key in flat_rows[j]
                ]

                if peers:
                    peer_mean = sum(peers) / len(peers)
                    consensus_z[key] = (
                        (peer_mean - info["mean"]) / scale
                        if info["learnable"]
                        else 0.0
                    )
                else:
            ...
            standardized_targets.append(target_z)
            peer_consensus.append(consensus_z)

        # Root-balanced feature weights.
        root_counts = {}
        for key in feature_keys:
            root_counts[root_of(key)] = root_counts.get(root_of(key), 0) + 1

        feature_weights = {
            key: 1.0 / root_counts[root_of(key)]
            for key in feature_keys
        }

        # One learnable coefficient per semantic/root feature family.
        # This prevents a long indexed sequence from receiving hundreds of
        # independent parameters.
        alpha = {root: 0.0 for root in roots}

        def reconstruction_for(target_index, parameters):
            result = {}
            target_consensus = peer_consensus[target_index]

            for key in feature_keys:
                root = root_of(key)
                a = parameters[root]
                result[key] = a * target_consensus[key]

            return result

        def loss_for(parameters):
            target_losses = []

            for target_index, target_z in enumerate(standardized_targets):
                reconstruction = reconstruction_for(
                    target_index, parameters
                )
                ...
                for key in feature_keys:
                    ...
                    error = target_z[key] - reconstruction[key]
            ...
            # Exact equal target/reference weighting.
            return sum(target_losses) / len(target_losses)

        initial_loss = loss_for(alpha)
        ...
        for iteration in range(1, max_iterations + 1):
            gradients = {root: 0.0 for root in roots}
            root_weight = {root: 0.0 for root in roots}

            # Exact gradient of the leave-one-out reconstruction objective.
            for target_index, target_z in enumerate(standardized_targets):
                consensus_z = peer_consensus[target_index]

                for key in feature_keys:
                    root = root_of(key)
                    weight = feature_weights[key]
                    error = target_z[key] - alpha[root] * consensus_z[key]

                    gradients[root] += (
                        -2.0 * weight * error * consensus_z[key]
                    )
            ...
            for root in roots:
                if root_weight[root]:
                    gradients[root] /= (
                        len(standardized_targets) * root_weight[root]
                    )
            ...
            # Backtracking line search, with alpha constrained to [0, 1].
            # alpha=0 means global style; alpha=1 means pure peer consensus.
            for _ in range(32):
                candidate = {
                    root: min(
                        2.0,
                        max(
                            -2.0,
                            alpha[root] - trial_lr * gradients[root],
                        ),
                    )
                    for root in roots
                }
                ...
                if (
                    ...
                    and candidate_loss <= previous_loss + 1e-15
                ):
                    alpha = candidate
            ...
            history.append({
                ...
                "diagnostic": {
                    ...
                    "alpha_min": min(alpha.values()) if alpha else 0.0,
                    "alpha_max": max(alpha.values()) if alpha else 0.0,
                    "alpha_mean": (
                        sum(alpha.values()) / len(alpha)
                        if alpha else 0.0
                    ),
                    "alpha_nonzero_count": sum(
                        1 for value in alpha.values()
                        if abs(value) > 1e-15
                    ),
                },
            })
        ...
        # Convert learned standardized alpha parameters into a readable
        # style-rule representation. This is knowledge, not MODEL v001.
        learned_rules = {}
        ...
        for root in roots:
            learned_rules[root] = {
                "alpha": alpha[root],
                "interpretation": (
                    "0 = global reference style; positive/negative values "
                    "blend or extrapolate toward/away from leave-one-out "
                    "peer consensus"
                ),
            }
        ...
        return {
            "version": "0.5",
            "type": "trajectory_style_leave_one_out_learning",
            "status": {
                "complete": True,
                "converged": converged,
            ...
            "learning_policy": {
                "objective":
                    "equal_weight_leave_one_out_trajectory_reconstruction",
                "optimizer":
                    "deterministic_gradient_descent_with_backtracking",
                "parameterization":
                    "per_root_consensus_blending_coefficient",
                ...
                "feature_count_learnable": len(feature_keys),
                "root_parameter_count": len(roots),
                "normalization": "mean_centered_robust_scale",
                "reference_weighting": "exactly_equal_1_over_25",
                "target_policy":
                    "target_excluded_from_its_own_peer_consensus",
            },
            "cycle_v0_1_baseline": {
                "loss": baseline,
            ...
            "learned_style": {
                "representation":
                    "trajectory_motion_rules_leave_one_out_v0.4",
                "learned_rules": learned_rules,
                "global_style": global_mean,
            ...
            "important_note": (
                "STEP 8 v0.5 learns deterministic trajectory style rules "
                "from all 25 frozen references using leave-one-out peer "
                "consensus. The target sample is never included in its own "
                "reconstruction. No video, random points, generated samples, "
                "MODEL v001, or Sample 026 are used or created. This is still "
                "a style-learning representation, not the final generative "
                "signature model."
            ),
        }
